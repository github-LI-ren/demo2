...
<head>
    ...
    <script type="text/javascript"
            src="${APP_PATH }/static/js/jquery-2.0.3.min.js"></script>
    <link
...
<script type="text/javascript">
    ...
    function build_page_info(result){
        ...
        pags=result.extend.pageInfo.pages;
    ...
    $("#empName_add_input").change(function(){
        ...
        //先判断用户名是否合法
        var regStuName = /^([\w\u2E80-\u9FFF]{2,5})$/;
        if (!regStuName.test(stuName)){
            show_validate_msg("#empName_add_input","error","正则检验用户名不可用")
            return false;
        }
        //发送ajax请求
        $.ajax({
            ...
            success:function(result){
                if(result.code==100){
                    ...
                    show_validate_msg("#empName_add_input","error","学生姓名不可用");
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                if(result.code == 100){
                    ...
                    //哪个字段有错误，显示哪个字段的错误
                    if(undefined != result.extend.emsg.email){
                        //显示邮箱的错误信息
                        show_validate_msg("#email_add_input", "error", result.extend.emsg.email);
                    }

                    if(undefined != result.extend.emsg.stuName){
                        //显示学生姓名的错误信息
                        show_validate_msg("#empName_add_input", "error", result.extend.emsg.stuName);
                    }
                }
            }
        });
    ...
    function getEmp(id){
        $.ajax({
            url:"${APP_PATH}/getStuById?id="+id,
            type:"GET",
            success:function(result){

                var empData = result.extend.stuData;
    ...
    $("#emp_update_btn").click(function(){
        ...
        //2. 发送ajax请求保存更新的学生数据
        $.ajax({
            url:"${APP_PATH}/putStu/"+$(this).attr("edit-id"),
